...
def init(ContextInfo):
    ...
    g.m_days = 25                           # 动量计算天数
    g.adjust_time = "11:25"                 # 每日调仓时间（实盘用）
    g.manual_capital = 30000                # 单只 ETF 最大投入金额（0=不限制）
    ...
    # ---------- 注册定时任务（仅实盘） ----------
    if g.run_mode == "live":
        # 动态计算下一个 11:15（确保 startTime 在未来，避免立即触发死循环）
...
def daily_rebalance(ContextInfo):
    ...
    # ---- 立即重新注册 run_time（计算下一个 11:15，确保不丢今天） ----
    if g.run_mode == "live":
        now = datetime.datetime.now()
        target = now.replace(hour=11, minute=25, second=0, microsecond=0)
        if now >= target:
    ...
    if g.run_mode == "live":
        ...
        if not DEBUG_MODE:
            ...
            # 仅在调仓时间窗口内执行（11:25-11:55）
            if not ("112500" <= now_time <= "115500"):
                return
...
def execute_rebalance(ContextInfo, targets, scores_df):
    """
    按目标调仓，先卖后买。
    仅当目标 ETF 与当前持仓不同时才操作；相同则什么都不做。
    targets: dict {etf: weight}
    买入金额不超过 g.manual_capital（若设置>0）。
    """
    ...
    print(f"【调仓决策】持仓: {current_etfs or '空仓'} → 目标: {target_etfs or '空仓'}，开始调仓")

    # ---- 1. 卖出：清仓所有 etf_pool 中的持仓 ----
    for etf, vol in pool_holdings.items():
        if vol > 0:
            sell_position(ContextInfo, etf, vol, "调出", price_dict)
...
def sell_position(ContextInfo, etf_code, volume, reason, price_dict=None):
    ...
    print(f"  卖出委托已发送：{etf_code} {vol}股")
...
def buy_position(ContextInfo, etf_code, buy_value, price_dict):
    ...
    print(f"  买入委托已发送：{etf_code} {vol}股")
