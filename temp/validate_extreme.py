#!/usr/bin/env python3
"""Target: find the extreme edge for return-heavy weight combos."""
...
INDEX = "000852"; START = "20230101"; END = "20260611"
...
def main():
    ...
    for label, roe, ret, rsi in COMBOS:
        weights = {factor_map["roe_yoy_rank"]: roe, factor_map["return_20d"]: ret, factor_map["rsi_14d"]: rsi}
        df_run = df.copy()
        ...
        m = bt.metrics
        print(f"{label:10s} roe={roe:+.2f} ret={ret:+.2f} rsi={rsi:+.2f}  "
              f"年化={m['annual_return']:.2%}  回撤={m['max_drawdown']:.2%}  "
              f"夏普={m['sharpe_ratio']:.3f}  胜率={m['win_rate']:.1%}  净值=¥{m['final_nav']/1e6:.2f}M", flush=True)
