...
多因子量化选股系统，支持全部A股、沪深300、中证500、中证1000成分股（可多选组合）及自定义股票池的因子分析与回测。
...
- **数据获取** — 支持 AKShare 和 Tushare 两种数据源，自动增量同步。Tushare 采用按日期全市场拉取，增量秒级完成。（AKShare会达到上限被封IP，Tushare可能需要充钱）
- **因子计算** — 动量、波动率、RSI、换手率、估值（PE/PB）、ROE改善等 11 个因子，可自由组合（因子不够自己加）
...
- **仓位管理** — 等权 / 评分加权 / 风险平价
- **行业中性** — 可选约束单行业持仓上限
- **方案配置** — 保存/加载因子组合与权重方案（`schemes.yaml`），内置 default / conservative / v1_zz1000 / momentum / 动量增强版0610 方案
- **自定义股票池** — `--stocks` 指定任意股票代码直接回测，跳过因子选股
...
```bash
# 沪深300 + 中证500 组合
python run.py --index 000300,000905 --start 20240101 --end 20260411 --source tushare --scheme momentum -y

# 全部A股（约5500只股票）
python run.py --index all --start 20240101 --end 20260411 --source tushare --scheme v1_zz1000 -y
```
...
```bash
# 指定股票代码直接等权回测（跳过因子选股）
python run.py --stocks 600036,601628,002475 --start 20230101 --end 20260426 --source tushare -y
```
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--index` | 股票池：000300 / 000905 / 000852 / all（多选用逗号分隔，与 --stocks 互斥） | 000300 |
| `--stocks` | 自定义股票代码，逗号分隔（与 --index 互斥） | 无 |
...
- **Tushare 按日期全市场拉取**：一次 API 调用获取当日所有股票数据，增量同步秒级完成
- **数据拉取不受股票范围影响**：无论选择什么指数，都会拉取全市场数据；股票范围仅用于下游计算和回测
- 回测使用 T+1 执行（信号日次日开盘价），避免前视偏差
- 风控参数通过 `--stop-loss` 等传入时自动启用风控
- `--stocks` 模式下自动跳过因子计算和评分，对全部指定股票等权回测
- `--stocks` 与 `--index` 不可同时使用，`--stocks` 会忽略 `--scheme`/`--factors`
- `--index all` 拉取全部A股（约5500只），`--index 000300,000905` 合并沪深300+中证500
